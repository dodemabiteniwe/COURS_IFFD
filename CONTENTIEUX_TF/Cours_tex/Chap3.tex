...
\section{Le principe du "Préalable Administratif Obligatoire"}
...
\subsection{Les conditions de délai : La forclusion (Art 374 LPF)}
...
\begin{itemize}
	...
	En matière d'impôts directs locaux (TFPB, TFPNB), le délai de réclamation expire le \textbf{31 décembre de l'année suivant} celle de la mise en recouvrement de l'impôt.
	\textit{Exemple : Pour un avis d'imposition reçu en juin 2025, la réclamation est recevable jusqu'au 31 décembre 2026.}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	\item \textbf{L'effet interruptif de prescription :} La réclamation interrompt la prescription triennale. Elle ouvre une phase d'instruction administrative durant laquelle les délais de recouvrement sont suspendus.
	
	\item \textbf{L'absence d'effet suspensif de paiement (Principe du "Payer d'abord, discuter ensuite"): } 
   C'est une règle cardinale du droit fiscal : \textbf{la réclamation ne suspend pas l'exigibilité de l'impôt}. Le comptable public est en droit de poursuivre le recouvrement forcé (saisie) même si une réclamation est en cours d'instruction, sauf si le contribuable a expressément formulé une demande de\textbf{ sursis de paiement assortie de garanties} (Voir \ref*{sec: sursi_paiement}).
\end{itemize}
...
\begin{itemize}
	\item \textbf{Le délai de réponse :} L'Administration dispose d'un délai légal (généralement \textbf{6 mois}, Art. 381 LPF) pour instruire et statuer.
...
\section{Le sursis de paiement (L'aménagement de l'exigibilité). Art. 519 LPF}
\label{sec: sursi_paiement}
